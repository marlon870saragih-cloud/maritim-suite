...
/**
 * Factory handler untuk Purchase Requisition & Purchase Order — struktur identik,
 * beda kind/docType/contoh. Dipakai route /api/documents/{pr,po}.
 */
export function makeProcurementHandlers(opts: { kind: ProcKind; docType: DocType }) {
  const { kind, docType } = opts
  const sample = kind === 'pr' ? SAMPLE_PR : SAMPLE_PO
  const fallbackNo = kind === 'pr' ? 'PR' : 'PO'

  function pdfResponse(data: ProcData, download: boolean) {
  ...
  function mergeData(body: Partial<ProcData>, tenant: EpdaTenant | null): ProcData {
    return {
      ...sample,
      ...body,
      kind,
      ...
      taxPct: Number.isFinite(body.taxPct) ? Number(body.taxPct) : sample.taxPct,
    }
  }
  ...
  async function GET(req: Request) {
    ...
    if (id) {
      ...
      const stored = (doc.lineItems ?? {}) as Partial<ProcData>
  ...
  async function POST(req: Request) {
    ...
    const download = url.searchParams.get('download') === '1'
    const body = (await req.json().catch(() => ({}))) as Partial<ProcData>
    const session = await getServerSession(authOptions)
    const tenant = await epdaTenantForSession(session?.user?.tenantId)
    ...
    if (save) {
      if (!session?.user) return new Response('Unauthorized', { status: 401 })
      const totals = computeProcTotals(data)
      const { tenant: _omit, ...payload } = data
      const fields = {
        docNumber: data.docNumber || fallbackNo,
        port: data.deliveryTo || null,
        currency: data.currency || 'IDR',
        lineItems: payload as object,
        subtotals: { subtotal: totals.subtotal, tax: totals.tax } as object,
