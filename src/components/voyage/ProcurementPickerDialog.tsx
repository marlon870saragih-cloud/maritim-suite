...
// "Ambil dari PO/WO" (K122, Fase 7i) — dua tahap: (1) pilih PO RECEIVED atau
// WO COMPLETED/VERIFIED voyage ini yang belum dipakai, (2) form baris
// TERISI tapi TETAP HARUS disimpan operator. Batalkan di tahap mana pun →
// TAK ADA DisbursementItem lahir (K122, pemeriksaan inti increment ini).
//
// PO/WO TIDAK PERNAH menulis baris biaya sendiri (K122/K52) — dialog ini
// cuma mengisi form, penyimpanannya lewat endpoint item ad-hoc yang SUDAH
// ADA (`POST /api/disbursements/[id]/items`, tanpa serviceId), plus dua
// field penanda opsional (sourcePurchaseOrderId/sourceWorkOrderId).
...
const STR: Record<Lang, Record<string, string>> = {
  id: {
    title: 'Ambil dari PO/WO', desc: 'Pilih Purchase Order yang sudah diterima atau Work Order yang sudah selesai — form baris terisi, tetap Anda yang menyimpan.',
    tabPo: 'Purchase Order', tabWo: 'Work Order',
  ...
  en: {
    title: 'Take from PO/WO', desc: 'Pick a received Purchase Order or completed Work Order — the line form is pre-filled, you still save it.',
    tabPo: 'Purchase Order', tabWo: 'Work Order',
...
export function ProcurementPickerDialog({
  ...
  const [picked, setPicked] = useState<{ source: 'po' | 'wo'; id: string; vendorId: string | null } | null>(null)
  ...
  useEffect(() => {
    ...
    setPicked(null)
    setError('')
    fetch(`/api/voyages/${voyageId}/procurement-sources`)
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((d: { po: PoOption[]; wo: WoOption[] }) => {
        setPo(d.po)
        setWo(d.wo)
      })
    ...
    try {
      const res = await fetch(`/api/disbursements/${disbursementId}/items`, {
        ...
        body: JSON.stringify({
          ...
          ...(picked.source === 'po' ? { sourcePurchaseOrderId: picked.id } : { sourceWorkOrderId: picked.id }),
        }),
      })
      if (!res.ok) {
