...
export function NoteProtestForm() {
  const { tenant: _t, ...sample } = SAMPLE_NOTEPROTEST
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
        <div className="space-y-5">
          <div>
            ...
            <h1 className="font-display text-2xl text-white">Buat Note of Protest</h1>
            <p className="text-text-secondary text-sm mt-1">Protes laut Nakhoda atas cuaca/laut buruk, dicatat di hadapan otoritas.</p>
          </div>

          {fromPortCall && (
            <div className="rounded-md border border-accent-teal/30 bg-accent-teal/5 px-4 py-2.5 text-xs text-accent-teal">
              Data kapal &amp; pelabuhan terisi otomatis dari Port Call. Lengkapi pelayaran &amp; isi protes.
            </div>
          )}

          <section className="bg-card-bg border border-card-border rounded-lg p-5">
            <h2 className="font-display text-base text-white mb-4">Kapal &amp; Pelayaran</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <Field label="No. NOP" value={form.docNumber} onChange={set('docNumber')} />
              <Field label="Tempat dicatat" value={form.place} onChange={set('place')} />
              <Field label="Tanggal" value={form.date} onChange={set('date')} />
              <Field label="Nakhoda" value={form.masterName} onChange={set('masterName')} />
              ...
              <Field label="Dari pelabuhan" value={form.fromPort} onChange={set('fromPort')} />
              <Field label="Ke pelabuhan" value={form.toPort} onChange={set('toPort')} />
              <Field label="Kargo" value={form.cargo} onChange={set('cargo')} />
              <Field label="Tgl berangkat" value={form.departureDate} onChange={set('departureDate')} />
              <Field label="Tgl tiba" value={form.arrivalDate} onChange={set('arrivalDate')} />
            </div>
          </section>

          <section className="bg-card-bg border border-card-border rounded-lg p-5">
            <h2 className="font-display text-base text-white mb-4">Isi Protes</h2>
            <div>
              <label className={labelCls}>Uraian kondisi (cuaca/laut yang dihadapi)</label>
              <textarea value={form.statement} onChange={(e) => set('statement')(e.target.value)} rows={5} className={inputCls} />
            </div>
            <div className="mt-3">
              <label className={labelCls}>Klausul reservasi hak</label>
              <textarea value={form.reservation} onChange={(e) => set('reservation')(e.target.value)} rows={4} className={inputCls} />
            </div>
            <div className="mt-3">
              <Field label="Dicatat di hadapan (Notaris/Syahbandar)" value={form.notedBefore} onChange={set('notedBefore')} />
            </div>
          </section>
        </div>
        ...
        <aside className="lg:sticky lg:top-5 space-y-3">
          <div className="bg-card-bg border border-card-border rounded-lg p-5">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between text-text-secondary"><span>Nakhoda</span><span className="text-text-primary text-right max-w-[60%] truncate">{form.masterName || '—'}</span></div>
