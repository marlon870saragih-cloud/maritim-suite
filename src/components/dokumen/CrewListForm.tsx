...
export function CrewListForm() {
  ...
  async function saveDraft() {
    setBusy('save')
    try {
      const res = await fetch(`/api/documents/crew-list?save=1${savedId ? `&id=${savedId}` : ''}`, {
        method: 'POST',
    ...
    <div className="p-margin-page max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
        <div className="space-y-5">
          <div>
            <p className="font-mono text-[10px] text-text-secondary uppercase tracking-widest mb-1">Maritime Dokumen · FAL Form 5</p>
            <h1 className="font-display text-2xl text-white">Buat Crew List</h1>
            <p className="text-text-secondary text-sm mt-1">Daftar awak kapal (IMO FAL 5) untuk kedatangan/keberangkatan.</p>
          </div>

          {fromPortCall && (
            <div className="rounded-md border border-accent-teal/30 bg-accent-teal/5 px-4 py-2.5 text-xs text-accent-teal">
              Data kapal terisi otomatis dari Port Call. Lengkapi daftar awak di bawah.
            </div>
          )}
          ...
          <section className="bg-card-bg border border-card-border rounded-lg p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-display text-base text-white">Daftar Awak</h2>
            ...
            <div className="hidden md:grid grid-cols-12 gap-2 px-1 mb-1.5 text-[9px] font-mono uppercase tracking-wider text-text-secondary/60">
              ...
              <div className="col-span-2">No. Paspor</div>
              <div className="col-span-2">Tgl Lahir</div>
              <div className="col-span-1" />
            </div>
            <div className="space-y-2">
              {crew.map((c, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-start">
                  <input value={c.name} onChange={(e) => updateCrew(i, 'name', e.target.value)} placeholder="Nama" className={`${inputCls} col-span-6 md:col-span-3`} />
                  <input value={c.rank} onChange={(e) => updateCrew(i, 'rank', e.target.value)} placeholder="Jabatan" className={`${inputCls} col-span-6 md:col-span-2`} />
                  <input value={c.nationality} onChange={(e) => updateCrew(i, 'nationality', e.target.value)} placeholder="Kebangsaan" className={`${inputCls} col-span-4 md:col-span-2`} />
                  <input value={c.passport} onChange={(e) => updateCrew(i, 'passport', e.target.value)} placeholder="Paspor" className={`${inputCls} col-span-4 md:col-span-2`} />
                  <input value={c.dob} onChange={(e) => updateCrew(i, 'dob', e.target.value)} placeholder="Tgl lahir" className={`${inputCls} col-span-3 md:col-span-2`} />
                  <button type="button" onClick={() => removeCrew(i)} aria-label="Hapus awak" className="col-span-1 flex items-center justify-center h-9 rounded text-text-secondary hover:text-status-danger hover:bg-status-danger/10 transition-colors">
        ...
        <aside className="lg:sticky lg:top-5 space-y-3">
          <div className="bg-card-bg border border-card-border rounded-lg p-5">
            ...
            <div className="mt-3 -mx-5 -mb-5 px-5 py-3 bg-[#0D2A50] border-t border-[#1D4A8A] rounded-b-lg">
              <p className="text-[10px] font-mono uppercase tracking-wider text-accent-blue/70">Total Awak</p>
              <p className="font-display text-xl text-white">{crew.length} orang</p>
            </div>
          </div>
