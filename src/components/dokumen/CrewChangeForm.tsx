...
export function CrewChangeForm() {
  const { tenant: _t, crew: _c, ...sampleHead } = SAMPLE_CREWCHANGE
  const [head, setHead] = useState<Head>(sampleHead)
  const [crew, setCrew] = useState<CrewChangeRow[]>(clone(SAMPLE_CREWCHANGE.crew))
  ...
  const onCount = crew.filter((c) => /on/i.test(c.action)).length
  const offCount = crew.filter((c) => /off/i.test(c.action)).length
    ...
    <div className="p-margin-page max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
        <div className="space-y-5">
          <div>
            <p className="font-mono text-[10px] text-text-secondary uppercase tracking-widest mb-1">Maritime Dokumen · Crew &amp; Husbandry</p>
            <h1 className="font-display text-2xl text-white">Buat Crew Change Notice</h1>
            <p className="text-text-secondary text-sm mt-1">Pemberitahuan pergantian awak (sign on/off) ke Imigrasi/Syahbandar/principal.</p>
          </div>

          {fromPortCall && (
            <div className="rounded-md border border-accent-teal/30 bg-accent-teal/5 px-4 py-2.5 text-xs text-accent-teal">
              Data kapal terisi otomatis dari Port Call. Lengkapi pihak tertuju &amp; daftar pergantian awak.
            </div>
          )}

          <section className="bg-card-bg border border-card-border rounded-lg p-5">
            <h2 className="font-display text-base text-white mb-4">Kapal &amp; Tujuan</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <Field label="Kepada (Imigrasi/Syahbandar)" value={head.toName} onChange={set('toName')} />
              <Field label="Attn" value={head.toAttn ?? ''} onChange={set('toAttn')} />
              <Field label="Penandatangan agen" value={head.agentName} onChange={set('agentName')} />
            </div>
          </section>

          <section className="bg-card-bg border border-card-border rounded-lg p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-display text-base text-white">Daftar Pergantian Awak</h2>
              <span className="text-xs font-mono text-text-secondary">{crew.length} orang</span>
            </div>
            <div className="hidden md:grid grid-cols-12 gap-2 px-1 mb-1.5 text-[9px] font-mono uppercase tracking-wider text-text-secondary/60">
              ...
              <div className="col-span-2">Kebangsaan</div>
              <div className="col-span-2">Paspor</div>
            ...
            <div className="space-y-2">
              {crew.map((c, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-start">
                  <input value={c.name} onChange={(e) => updateRow(i, 'name', e.target.value)} placeholder="Nama" className={`${inputCls} col-span-6 md:col-span-3`} />
                  <input value={c.rank} onChange={(e) => updateRow(i, 'rank', e.target.value)} placeholder="Jabatan" className={`${inputCls} col-span-6 md:col-span-2`} />
                  <input value={c.nationality} onChange={(e) => updateRow(i, 'nationality', e.target.value)} placeholder="Kebangsaan" className={`${inputCls} col-span-4 md:col-span-2`} />
                  <input value={c.passport} onChange={(e) => updateRow(i, 'passport', e.target.value)} placeholder="Paspor" className={`${inputCls} col-span-4 md:col-span-2`} />
                  <select value={/off/i.test(c.action) ? 'Sign Off' : 'Sign On'} onChange={(e) => updateRow(i, 'action', e.target.value)} className={`${inputCls} col-span-3 md:col-span-2`}>
                  ...
                  <input value={c.remark ?? ''} onChange={(e) => updateRow(i, 'remark', e.target.value)} placeholder="Remark (detail penerbangan, dll.)" className={`${inputCls} col-span-12`} />
                </div>
              ))}
            </div>
