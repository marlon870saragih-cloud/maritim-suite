      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
        <div className="space-y-5">
          <div>
            ...
            <h1 className="font-display text-2xl text-white">Buat Agency Appointment</h1>
            <p className="text-text-secondary text-sm mt-1">Surat penunjukan/konfirmasi keagenan untuk port call.</p>
          </div>

          {fromPortCall && (
            <div className="rounded-md border border-accent-teal/30 bg-accent-teal/5 px-4 py-2.5 text-xs text-accent-teal">
              Data kapal &amp; principal terisi otomatis dari Port Call. Sesuaikan lingkup layanan.
            </div>
          )}

          <section className="bg-card-bg border border-card-border rounded-lg p-5">
            <h2 className="font-display text-base text-white mb-4">Kepada &amp; Kapal</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <Field label="Kepada (principal)" value={head.toName} onChange={set('toName')} />
              <Field label="Attn" value={head.toAttn ?? ''} onChange={set('toAttn')} />
              <div className="col-span-2 md:col-span-1">
                <Field label="Alamat" value={head.toAddress ?? ''} onChange={set('toAddress')} />
              </div>
          ...
          <section className="bg-card-bg border border-card-border rounded-lg p-5">
            <h2 className="font-display text-base text-white mb-4">Isi Surat</h2>
            <div>
              <label className={labelCls}>Paragraf pembuka</label>
              <textarea value={head.intro} onChange={(e) => set('intro')(e.target.value)} rows={3} className={inputCls} />
            </div>
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                <label className={labelCls + ' mb-0'}>Lingkup layanan</label>
                <span className="text-xs font-mono text-text-secondary">{scope.length} poin</span>
              </div>
              <div className="space-y-2">
                {scope.map((s, i) => (
                  <div key={i} className="flex gap-2 items-start">
                    <span className="text-accent-amber font-mono text-sm pt-2 w-5">{i + 1}.</span>
                    <input value={s} onChange={(e) => updateScope(i, e.target.value)} placeholder="Layanan…" className={inputCls} />
                    <button type="button" onClick={() => removeScope(i)} aria-label="Hapus" className="flex items-center justify-center h-9 w-9 flex-none rounded text-text-secondary hover:text-status-danger hover:bg-status-danger/10 transition-colors">
            ...
            <div className="mt-4">
              <label className={labelCls}>Masa berlaku</label>
              <input value={head.validity} onChange={(e) => set('validity')(e.target.value)} className={inputCls} />
            </div>
            <div className="grid grid-cols-2 gap-3 mt-3">
              <Field label="Penandatangan" value={head.signName} onChange={set('signName')} />
              <Field label="Jabatan" value={head.signRole} onChange={set('signRole')} />
            </div>
          </section>
        </div>
        ...
        <aside className="lg:sticky lg:top-5 space-y-3">
          <div className="bg-card-bg border border-card-border rounded-lg p-5">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-text-secondary"><span>Principal</span><span className="text-text-primary text-right max-w-[60%] truncate">{head.toName || '—'}</span></div>
              ...
              <div className="flex justify-between text-text-secondary"><span>Lingkup</span><span className="font-mono text-text-primary">{scope.length} poin</span></div>
            </div>
          </div>
