...
import { CrewListDocument } from '@/lib/pdf/CrewListDocument'
import { SAMPLE_CREWLIST, type CrewListData } from '@/lib/pdf/crewlist-data'
...
const DOC_TYPE = 'FAL_5' as const

function pdfResponse(data: CrewListData, download: boolean) {
  const filename = `${(data.docNumber || 'CREW-LIST').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(CrewListDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<CrewListData>, tenant: EpdaTenant | null): CrewListData {
  return {
    ...SAMPLE_CREWLIST,
    ...body,
    tenant: tenant ?? SAMPLE_CREWLIST.tenant,
    crew: Array.isArray(body.crew) ? body.crew : SAMPLE_CREWLIST.crew,
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<CrewListData>
  ...
  return pdfResponse(mergeData(SAMPLE_CREWLIST, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<CrewListData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'CREW-LIST',
      port: data.port || null,
      currency: 'IDR',
