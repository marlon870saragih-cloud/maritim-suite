...
import { PortCallSummaryDocument } from '@/lib/pdf/PortCallSummaryDocument'
import { SAMPLE_PCSUMMARY, type PortCallSummaryData } from '@/lib/pdf/pcsummary-data'
...
const DOC_TYPE = 'PORT_CALL_SUMMARY' as const

function pdfResponse(data: PortCallSummaryData, download: boolean) {
  const filename = `${(data.docNumber || 'PCS').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(PortCallSummaryDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<PortCallSummaryData>, tenant: EpdaTenant | null): PortCallSummaryData {
  return {
    ...SAMPLE_PCSUMMARY,
    ...body,
    tenant: tenant ?? SAMPLE_PCSUMMARY.tenant,
    documents: Array.isArray(body.documents) ? body.documents : SAMPLE_PCSUMMARY.documents,
    finance: { ...SAMPLE_PCSUMMARY.finance, ...(body.finance ?? {}) },
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<PortCallSummaryData>
  ...
  return pdfResponse(mergeData(SAMPLE_PCSUMMARY, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<PortCallSummaryData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
