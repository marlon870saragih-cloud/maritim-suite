...
import { DamageDocument } from '@/lib/pdf/DamageDocument'
import { SAMPLE_DAMAGE, damageTotal, type DamageData } from '@/lib/pdf/damage-data'
...
const DOC_TYPE = 'DAMAGE_REPORT' as const

function pdfResponse(data: DamageData, download: boolean) {
  const filename = `${(data.docNumber || 'DMG').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(DamageDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<DamageData>, tenant: EpdaTenant | null): DamageData {
  return {
    ...SAMPLE_DAMAGE,
    ...body,
    tenant: tenant ?? SAMPLE_DAMAGE.tenant,
    items: Array.isArray(body.items) ? body.items : SAMPLE_DAMAGE.items,
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<DamageData>
  ...
  return pdfResponse(mergeData(SAMPLE_DAMAGE, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<DamageData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'DMG',
      port: data.port || null,
      currency: data.currency || 'IDR',
      lineItems: payload as object,
      grandTotal: damageTotal(data),
    }
