...
import { LoiDocument } from '@/lib/pdf/LoiDocument'
...
const DOC_TYPE = 'LETTER_OF_INDEMNITY' as const

function pdfResponse(data: LoiData, download: boolean) {
  const filename = `${(data.docNumber || 'LOI').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(LoiDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<LoiData>, tenant: EpdaTenant | null): LoiData {
  return { ...SAMPLE_LOI, ...body, tenant: tenant ?? SAMPLE_LOI.tenant }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<LoiData>
  ...
  return pdfResponse(mergeData(SAMPLE_LOI, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<LoiData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
