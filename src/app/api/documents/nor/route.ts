...
const DOC_TYPE = 'NOR' as const

function pdfResponse(data: NorData, download: boolean) {
  const filename = `${(data.docNumber || 'NOR').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(NorDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<NorData>, tenant: EpdaTenant | null): NorData {
  return { ...SAMPLE_NOR, ...body, tenant: tenant ?? SAMPLE_NOR.tenant }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<NorData>
  ...
  return pdfResponse(mergeData(SAMPLE_NOR, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<NorData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
