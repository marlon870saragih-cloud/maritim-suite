...
import { AppointmentDocument } from '@/lib/pdf/AppointmentDocument'
...
const DOC_TYPE = 'AGENCY_APPOINTMENT' as const

function pdfResponse(data: AppointmentData, download: boolean) {
  const filename = `${(data.docNumber || 'AGENCY-APPOINTMENT').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(AppointmentDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<AppointmentData>, tenant: EpdaTenant | null): AppointmentData {
  return {
    ...SAMPLE_APPOINTMENT,
    ...body,
    tenant: tenant ?? SAMPLE_APPOINTMENT.tenant,
    scope: Array.isArray(body.scope) ? body.scope : SAMPLE_APPOINTMENT.scope,
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<AppointmentData>
  ...
  return pdfResponse(mergeData(SAMPLE_APPOINTMENT, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<AppointmentData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'AGENCY-APPOINTMENT',
      port: data.port || null,
      currency: 'IDR',
