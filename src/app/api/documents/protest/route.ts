...
import { ProtestDocument } from '@/lib/pdf/ProtestDocument'
...
const DOC_TYPE = 'LETTER_OF_PROTEST' as const

function pdfResponse(data: ProtestData, download: boolean) {
  const filename = `${(data.docNumber || 'LOP').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(ProtestDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<ProtestData>, tenant: EpdaTenant | null): ProtestData {
  return { ...SAMPLE_PROTEST, ...body, tenant: tenant ?? SAMPLE_PROTEST.tenant }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<ProtestData>
  ...
  return pdfResponse(mergeData(SAMPLE_PROTEST, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<ProtestData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
