...
import { UllageDocument } from '@/lib/pdf/UllageDocument'
import { SAMPLE_ULLAGE, ullageTotalMt, type UllageData } from '@/lib/pdf/ullage-data'
...
const DOC_TYPE = 'ULLAGE_REPORT' as const

function pdfResponse(data: UllageData, download: boolean) {
  const filename = `${(data.docNumber || 'ULL').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(UllageDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<UllageData>, tenant: EpdaTenant | null): UllageData {
  return {
    ...SAMPLE_ULLAGE,
    ...body,
    tenant: tenant ?? SAMPLE_ULLAGE.tenant,
    tanks: Array.isArray(body.tanks) ? body.tanks : SAMPLE_ULLAGE.tanks,
    densityKgL: Number.isFinite(body.densityKgL) ? Number(body.densityKgL) : SAMPLE_ULLAGE.densityKgL,
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<UllageData>
  ...
  return pdfResponse(mergeData(SAMPLE_ULLAGE, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<UllageData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'ULL',
      port: data.port || null,
      currency: 'IDR',
      lineItems: payload as object,
      grandTotal: ullageTotalMt(data), // MT sebagai metrik utama
    }
