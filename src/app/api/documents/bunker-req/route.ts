...
import { BunkerReqDocument } from '@/lib/pdf/BunkerReqDocument'
import { SAMPLE_BUNKERREQ, bunkerTotal, type BunkerReqData } from '@/lib/pdf/bunkerreq-data'
...
const DOC_TYPE = 'BUNKER_REQUISITION' as const

function pdfResponse(data: BunkerReqData, download: boolean) {
  const filename = `${(data.docNumber || 'BRQ').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(BunkerReqDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<BunkerReqData>, tenant: EpdaTenant | null): BunkerReqData {
  return {
    ...SAMPLE_BUNKERREQ,
    ...body,
    tenant: tenant ?? SAMPLE_BUNKERREQ.tenant,
    lines: Array.isArray(body.lines) ? body.lines : SAMPLE_BUNKERREQ.lines,
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<BunkerReqData>
  ...
  return pdfResponse(mergeData(SAMPLE_BUNKERREQ, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<BunkerReqData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'BRQ',
      port: data.port || null,
      currency: data.currency || 'IDR',
      lineItems: payload as object,
      grandTotal: bunkerTotal(data),
    }
