...
import { ShipStoresDocument } from '@/lib/pdf/ShipStoresDocument'
import { SAMPLE_SHIPSTORES, type ShipStoresData } from '@/lib/pdf/shipstores-data'
...
const DOC_TYPE = 'FAL_3' as const

function pdfResponse(data: ShipStoresData, download: boolean) {
  const filename = `${(data.docNumber || 'SHIP-STORES').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(ShipStoresDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<ShipStoresData>, tenant: EpdaTenant | null): ShipStoresData {
  return {
    ...SAMPLE_SHIPSTORES,
    ...body,
    tenant: tenant ?? SAMPLE_SHIPSTORES.tenant,
    stores: Array.isArray(body.stores) ? body.stores : SAMPLE_SHIPSTORES.stores,
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<ShipStoresData>
  ...
  return pdfResponse(mergeData(SAMPLE_SHIPSTORES, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<ShipStoresData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'SHIP-STORES',
      port: data.port || null,
      currency: 'IDR',
