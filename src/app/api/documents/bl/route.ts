...
const DOC_TYPE = 'BILL_OF_LADING' as const

function pdfResponse(data: BlData, download: boolean) {
  const filename = `${(data.docNumber || 'BL').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(BlDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<BlData>, tenant: EpdaTenant | null): BlData {
  return { ...SAMPLE_BL, ...body, tenant: tenant ?? SAMPLE_BL.tenant }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<BlData>
  ...
  return pdfResponse(mergeData(SAMPLE_BL, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<BlData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'BL',
      port: data.portOfLoading || null,
      currency: 'IDR',
