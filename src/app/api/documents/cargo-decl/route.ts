...
import { CargoDeclDocument } from '@/lib/pdf/CargoDeclDocument'
import { SAMPLE_CARGO, type CargoDeclData } from '@/lib/pdf/cargo-data'
...
const DOC_TYPE = 'FAL_2' as const

function pdfResponse(data: CargoDeclData, download: boolean) {
  const filename = `${(data.docNumber || 'CARGO-DECLARATION').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(CargoDeclDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<CargoDeclData>, tenant: EpdaTenant | null): CargoDeclData {
  return {
    ...SAMPLE_CARGO,
    ...body,
    tenant: tenant ?? SAMPLE_CARGO.tenant,
    items: Array.isArray(body.items) ? body.items : SAMPLE_CARGO.items,
  }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<CargoDeclData>
  ...
  return pdfResponse(mergeData(SAMPLE_CARGO, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<CargoDeclData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
  ...
  if (save) {
    ...
    const fields = {
      docNumber: data.docNumber || 'CARGO-DECLARATION',
      port: data.port || null,
      currency: 'IDR',
