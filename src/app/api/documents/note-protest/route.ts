...
import { NoteProtestDocument } from '@/lib/pdf/NoteProtestDocument'
...
const DOC_TYPE = 'NOTE_OF_PROTEST' as const

function pdfResponse(data: NoteProtestData, download: boolean) {
  const filename = `${(data.docNumber || 'NOP').replace(/[\\/]/g, '-')}.pdf`
  const element = React.createElement(NoteProtestDocument, { data }) as React.ReactElement<DocumentProps>
...
function mergeData(body: Partial<NoteProtestData>, tenant: EpdaTenant | null): NoteProtestData {
  return { ...SAMPLE_NOTEPROTEST, ...body, tenant: tenant ?? SAMPLE_NOTEPROTEST.tenant }
}
...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    const stored = (doc.lineItems ?? {}) as Partial<NoteProtestData>
  ...
  return pdfResponse(mergeData(SAMPLE_NOTEPROTEST, tenant), download)
}
...
export async function POST(req: Request) {
  ...
  const download = url.searchParams.get('download') === '1'
  const body = (await req.json().catch(() => ({}))) as Partial<NoteProtestData>
  const session = await getServerSession(authOptions)
  const tenant = await epdaTenantForSession(session?.user?.tenantId)
