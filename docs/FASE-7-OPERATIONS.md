...
### K103 — Eskalasi: **satu tingkat**, hanya untuk `SLA_BREACH`

⚠️ **DIPERBARUI sesudah P34 dijawab final (17 Ags 2026) — bagian ini menggantikan rencana "siaran ke seluruh tenant" di bawah, bukan melengkapinya.**

Tugas yang terlambat mengirim pengingat ke penanggung jawabnya. Tugas yang **sudah dilanggar SLA-nya** mengirim eskalasi **bertarget hanya ke peran `ADMIN` dan `MANAJER_OPERASI`** — BUKAN siaran ke seluruh tenant. `MANAJER_OPERASI`/`DIREKTUR` versi lama dokumen ini digantikan: `DIREKTUR` **tidak** menerima eskalasi (ia lihat-saja, bukan penindak — konsisten K98/Fase 5e), `VIEWER`/`OPERATOR`/`PENYUSUN_BIAYA`/`FINANCE` juga tidak.

**Cara mengimplementasikan tanpa skema baru (K86 tetap berlaku — tak ada sistem notifikasi kedua):** bukan satu baris `Notification` ber-`userId=null` (broadcast), melainkan **satu baris per pengguna aktif** yang perannya `ADMIN` atau `MANAJER_OPERASI` di tenant itu — pola yang SAMA dengan bagaimana `@sebut` (K128, Comment) sudah menerbitkan notifikasi bertarget per orang. Idempotensi (K101) tetap lewat `dedupeKey`, tapi kuncinya kini menyertakan `userId` (satu dedupeKey per pasangan tugas+penerima), bukan satu dedupeKey global untuk seluruh siaran.

Yang **tidak** dibangun: eskalasi berjenjang (tingkat 1 → 2 → 3 dengan penerima berbeda per tingkat) — **P34 final tetap satu tingkat**, karena mekanisme berjenjang yang salah kalibrasi menghasilkan kebisingan yang persis melatih orang mengabaikan lonceng (alasan yang sama sudah dipakai P24 untuk anomali).
...
✅ **P32-P35 SEMUA SUDAH DIJAWAB FINAL (17 Ags 2026) — 7e tidak lagi berstatus interim.** `slaHours` bawaan tetap `null` per P32 (keputusan final, bukan sekadar sementara), ambang mendekati tetap 12 jam, jam kalender 24/7 per P33, eskalasi SATU tingkat bertarget ADMIN+MANAJER_OPERASI per P34 (lihat K103 versi terbaru — BUKAN lagi siaran ke seluruh tenant), kanal tetap lonceng aplikasi saja per P35 (WhatsApp dicatat sebagai keinginan nyata untuk fase mendatang, tidak dibangun sekarang).
...
5. Tugas selesai melewati tenggat → `SLA_BREACH` **sekali seumur hidup tugas** + satu notifikasi eskalasi **per pengguna aktif ber-peran ADMIN/MANAJER_OPERASI** (K103 versi P34 final — BUKAN satu siaran `userId=null`; periksa jumlah baris = jumlah pengguna ADMIN+MANAJER_OPERASI aktif tenant itu, masing-masing ber-`userId` sendiri).
...
| **P34** | **Eskalasi: ke siapa, setelah berapa lama, dan berapa tingkat?** Notifikasi berambang yang belum dikalibrasi akan melatih orang mengabaikan lonceng — pelajaran yang sama sudah dicatat di P24 | Satu tingkat, siaran ke seluruh tenant, hanya untuk `SLA_BREACH` (K103). **✅ KONFIRMASI (17 Ags) — FINAL:** DIPERSEMPIT dari interim K103 — bukan siaran ke seluruh tenant, melainkan bertarget hanya ke peran `ADMIN` dan `MANAJER_OPERASI` (dua peran yang memang berwenang menindaklanjuti, sejalan pemetaan K98). Tetap **satu tingkat** (bukan bertingkat) | 7e — **tidak lagi memblokir, FINAL** |
| **P35** | **Pengingat disampaikan lewat apa selain lonceng dalam aplikasi?** Kalau tim tidak membuka aplikasi terus-menerus, lonceng saja tidak sampai. WhatsApp adalah kanal nyata mereka — tapi itu integrasi baru (biaya, nomor bisnis, penyedia) dan bukan cakupan Fase 7. Terkait **P10** | Lonceng dalam aplikasi saja (K86). **✅ KONFIRMASI (17 Ags) — FINAL untuk Fase 7:** tetap lonceng aplikasi saja. Marlon MENGKONFIRMASI ingin WhatsApp juga suatu saat — dicatat sebagai kebutuhan nyata untuk direncanakan (kandidat Fase 8/lanjutan bersama P10), TIDAK dibangun sekarang | 7e — **tidak lagi memblokir, FINAL** |
