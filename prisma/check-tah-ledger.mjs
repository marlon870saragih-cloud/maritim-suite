...
const MC = muat('src/lib/ai/model-capabilities.ts')
{
  ...
  cek('identitas prompt: id/versi/hash & skema', tercatat.every((m) => m.promptId === 'vessel-call-extract' && m.promptVersion === '3' && m.promptVersion === X.VERSI_PROMPT_INTAKE && /^[0-9a-f]{64}$/.test(m.promptHash) && m.schemaId === 'isi_intake_kunjungan' && m.schemaVersion === '3' && m.schemaVersion === X.VERSI_SKEMA_INTAKE && m.provider === 'OPENROUTER'))
